...
async function renderCanvas() {
    ...
    const wasMobile = window.lastMobileState;
    const mobileStateChanged = wasMobile !== undefined && wasMobile !== isMobile;
    window.lastMobileState = isMobile;

    // Optimization: Update content in-place if page structure hasn't changed
    // AND we haven't crossed the mobile/desktop breakpoint
    ...
    if (pageFlip && pages.length > 0 && domPages.length === pages.length && !mobileStateChanged) {
    ...
    // Full re-render needed (page count changed, initial load, or mobile state changed)
    if (mobileStateChanged) {
        console.log(`renderCanvas: Mobile state changed (${wasMobile} -> ${isMobile}), forcing full re-render`);
    }
...
function initPageFlip() {
    ...
    // or just single page mode. 
    // PageFlip "usePortrait" true means single page view.
    ...
    if (isMobile) {
        // Single page view: page takes most of the width
        let targetWidth = availableWidth * 0.95;
        pageWidth = Math.floor(targetWidth);

        // Logical constraints for mobile
        pageWidth = Math.min(pageWidth, 800); // Don't get too wide on tablets
        pageHeight = Math.floor(pageWidth * 1.41); // A4 ratio approx

        // Height check
        const maxHeight = availableHeight * 0.95;
        if (pageHeight > maxHeight) {
            pageHeight = maxHeight;
            pageWidth = Math.floor(pageHeight / 1.41);
        }
    } else {
        // Double page view (Desktop)
        // Target spread width: 95% of available space
        let targetSpreadWidth = availableWidth * 0.95;

        // Page width is half of spread
        pageWidth = Math.floor(targetSpreadWidth / 2);

        // Maintain aspect ratio (e.g., 3:4 or A4)
        pageHeight = Math.floor(pageWidth * 1.33);
        ...
        // Ensure logical minimums for desktop
        pageWidth = Math.max(400, pageWidth);
        pageHeight = Math.max(600, pageHeight);
    }

    // Update blocker dimensions
    const spreadWidth = isMobile ? pageWidth : pageWidth * 2;
    ...
    try {
        pageFlip = new St.PageFlip(bookEl, {
            ...
            usePortrait: isMobile, // TRUE for single page (mobile), FALSE for double (desktop)
